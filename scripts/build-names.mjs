...
// BUILD_MAP_SEED=off лечит это по просьбе: семя качается как обычно и остаётся
// базой порога приёмки, но в волну не идёт, и все номера спрашиваются
// заново. Стоит это около шестисот десяти запросов и двадцати минут,
// поэтому расписанию такое ни к чему — только кнопке.
//
...
/**
 * Пересборка карты по просьбе: BUILD_MAP_SEED=off оставляет семя базой порога
 * приёмки, но в волну его не отдаёт — пары спрашиваются у AniList заново, все
 * до одной. Нужна затем, что иначе ядро карты не обновляется никогда,
 ...
 *
 * Качается даже при пересборке, когда в волну его не отдадут: без него нечем
 * страховать порог приёмки карты, а пересборка — ровно тот случай, где он
 * нужнее всего.
 */
...
async function main() {
  ...
  // Пересборка без волны — это пустая карта и падение на пороге приёмки
  // ниже. Просьбы, отменяющие друг друга, разбираются здесь: волна выключена,
  // значит пересобирать нечем, и семя остаётся на месте.
  ...
  if (MAP_REBUILD && !rebuild) {
    ...
    console.log('Карта: пересборка по настройке, семя в волну не идёт')
  }

  // При пересборке семя волне не отдаётся, но остаётся базой порога приёмки
  // ниже: недособранная пересборка не вправе уехать в выпуск картой
  // меньше прошлой.
  const map = await enrichMap(malIds, seed === null || rebuild ? [] : seed.pairs)
  ...
  //
  // Пересборку он же и страхует: семя в волну не пошло, но сравнение с ним
  // осталось, и пересборка, свернувшаяся на середине или упёршаяся в
  // закрытый доступ, роняет прогон вместо того, чтобы выпустить обрубок карты.
  if (baseline !== null) {
    if (seed !== null) {
