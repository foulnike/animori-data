...
async function main() {
  ...
  //
  // license сменилась с ODbL-1.0 на MIT. ODbL стояла не по выбору: она
  // приезжала вместе с производностью от манами, а производности больше нет.
  // Ни Шикимори, ни anime365, ни AniList условий на выгрузку через открытый
  // API не накладывают, поэтому условия распространения выбираем мы сами.
  const index = {
    version: 1,
    builtAt,
    source: 'shikimori',
  ...
  writeFileSync(
    ...
    [
      ...
      'Датасет и код сборщика выходят на условиях MIT.',
      '',
